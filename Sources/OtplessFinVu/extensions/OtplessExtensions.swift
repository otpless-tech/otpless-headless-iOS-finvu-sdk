...
//  OtplessSDK
//
//  Created by OTPless on 16/02/25.
//
...
extension Otpless {
    ...
    private func manageFBSignIn(with sdkAuthParams: SdkAuthParams) async {
        if let FacebookAuthClass = NSClassFromString("OtplessFinVu.FBSdkUseCase") as? NSObject.Type {
            ...
            if let handler = fbAuthHandler as? FacebookAuthProtocol {
                ...
                let errorDictionary = [
                    ...
                    "errorDescription": "Facebook support not initialized. Please add OtplessFinVu/FacebookSupport to your Podfile"
                ]
    ...
    private func manageGIDSignIn(with sdkAuthParams: SdkAuthParams) async {
        ...
        if let GoogleAuthClass = NSClassFromString("OtplessFinVu.GIDSignInUseCase") as? NSObject.Type {
            ...
            if let handler = googleAuthHandler as? GoogleAuthProtocol {
                ...
                let errorDictionary: [String: Any] = [
                    "error": "missing_dependency",
                    "error_description": "Google support not initialized. Please add OtplessFinVu/GoogleSupport to your Podfile"
                ]
                await verifySdkAuthResponse(queryParams: errorDictionary)
            }
        } else {
